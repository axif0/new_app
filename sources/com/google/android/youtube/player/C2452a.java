package com.google.android.youtube.player;

import android.content.Context;
import android.content.pm.PackageInfo;
import android.content.pm.PackageManager;
import android.content.pm.Signature;
import android.util.Base64;
import com.google.android.youtube.player.p042f.C2497r;
import java.io.ByteArrayInputStream;
import java.security.cert.CertificateException;
import java.security.cert.CertificateFactory;
import java.security.cert.X509Certificate;
import java.util.Arrays;

/* renamed from: com.google.android.youtube.player.a */
public final class C2452a {

    /* renamed from: a */
    private static final byte[][] f8087a = {Base64.decode("", 0), Base64.decode("", 0), Base64.decode("", 0), Base64.decode("", 0), Base64.decode("", 0), Base64.decode("", 0)};

    /* renamed from: a */
    private static boolean m11124a(PackageInfo packageInfo) {
        try {
            ((X509Certificate) CertificateFactory.getInstance("X509").generateCertificate(new ByteArrayInputStream(packageInfo.signatures[0].toByteArray()))).checkValidity();
            Signature[] signatureArr = packageInfo.signatures;
            if (signatureArr.length != 1) {
                return false;
            }
            byte[] byteArray = signatureArr[0].toByteArray();
            int i = 0;
            while (true) {
                byte[][] bArr = f8087a;
                if (i >= bArr.length) {
                    break;
                } else if (Arrays.equals(bArr[i], byteArray)) {
                    return true;
                } else {
                    i++;
                }
            }
            return false;
        } catch (CertificateException unused) {
        }
    }

    /* renamed from: b */
    public static C2453b m11125b(Context context) {
        try {
            PackageManager packageManager = context.getPackageManager();
            String c = C2497r.m11230c(context);
            return !m11124a(packageManager.getPackageInfo(c, 64)) ? C2453b.SERVICE_INVALID : C2497r.m11231d(context, c) ? C2453b.SERVICE_VERSION_UPDATE_REQUIRED : !packageManager.getApplicationInfo(c, 0).enabled ? C2453b.SERVICE_DISABLED : C2453b.SUCCESS;
        } catch (PackageManager.NameNotFoundException unused) {
            return C2453b.SERVICE_MISSING;
        }
    }
}
